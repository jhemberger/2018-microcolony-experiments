...
##### Model specification
```{r}
mc.feed.all.fix.df$rep <- as.factor(mc.feed.all.fix.df$rep)
mc.feed.all.fix.df$trt_pulse <- as.factor(mc.feed.all.fix.df$trt_pulse)
mc.feed.all.fix.df$trt_total <- as.factor(mc.feed.all.fix.df$trt_total)
...
drones.lmm <- lme(total_drones ~ trt_pulse * trt_total * rep,
                  ...
                  na.action = na.exclude)
drones.lmer <- lmer(total_drones ~ trt_pulse * trt_total * rep + 
                      (1 | id),
                    data = mc.drone.all.df,
                    na.action = na.exclude)
...
##### Model specification
```{r}
dronemass.lmm <- lme(avg.individ.mass ~ trt_total * trt_pulse * rep,
                     random = ~ 1 | id,
                     data = mc.dronemass.all.df,
                     na.action = na.exclude)
dronemass.lmer <- lmer(avg.individ.mass ~ trt_total * trt_pulse * rep + 
                         (1 | id),
                       data = mc.dronemass.all.df,
                       na.action = na.exclude)
...
dronemass.pairwise <- contrast(dronemass.marginal,
                               ...
                               method = "pairwise",
                               adjust = "tukey")
summary(dronemass.lmm)
anova(dronemass.lmm)
Anova(dronemass.lmm, type = 2)
Anova(dronemass.lmer, test.statistic = "F", type = 3)
dronemass.cld
dronemass.eff
dronemass.pairwise
```
...
ggplot(data = dronemass.cld,
  ...
  #          alpha = 1,
  #          color = "gray10") + 
  geom_jitter(data = mc.dronemass.all.df %>%
  ...
  scale_y_continuous(limits = c(0, .2),
                     expand = c(0, 0)) +
  ylab("LS mean mass per drone (grams)") + 
  xlab("Treatment") + 
  scale_color_manual(values = c("#9FC3EA", 
                               "#172A3A", 
                               "#E6AF2E",
...
##### Model specification
```{r}
droneit.lmm <- lme(it.distance ~ trt_total * trt_pulse * rep,
                  random = ~1 | id,
...
pcons.rmlmm.1 = lme(total_p_cons ~ trt_total * trt_pulse * rep + date, 
                    ...
                    na.action = na.exclude,
                    method = "REML")
pcons.gwm.df <- groupwiseMean(total_p_cons ~ trt_total * trt_pulse + fd_day,
                              data = mc.feed.all.cum.df,
                              conf = 0.95,
                              digits = 3)
...
### End Mass
```{r}
Anova(endmass.lmm, type = 3, test.statistic = "F")
anova(endmass.lmm)
```
