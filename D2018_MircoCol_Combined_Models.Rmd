...
mc1.drone.it.df <- read_csv("./D2018_microcol_round1_droneIT_clean.csv") %>%
  select(id, date, it.distance) %>%
  mutate(notes = NA) %>%
...
mc2.drone.df <- read_csv("./D2018_microcol")
mc2.drone.it.df <- read_csv("./D2018_microcol_round2_droneIT.csv",
                            skip = 1,
                            col_names = c("id",
                                          "date",
                                          "it.distance",
  ...
  mutate(trt_pulse = ifelse(id >= 3, 
         ...
         trt_total = ifelse(id < 2 | id > 4,
                            "60",
                            "100"))
mc2.drone.it.df$date <- mdy(mc2.drone.it.df$date)
...
mc2.drone.mass.df <- mc2.drone.mass.df %>%
  select(-notes)
...
mc3.drone.it.df <- read_csv("./D2018_microcol_round3_droneIT.csv",
                            skip = 1,
                            col_names = c("id",
                                          "date",
                                          "it.distance",
  ...
  mutate(trt_pulse = ifelse(id >= 3, 
         ...
         trt_total = ifelse(id < 2 | id > 4,
                            "60",
                            "100"))
mc3.drone.it.df$date <- mdy(mc3.drone.it.df$date)
...
mc3.drone.mass.df <- mc3.drone.mass.df %>%
  select(-notes)
```
...
mc3.feed.df$id <- as.character(mc3.feed.df$id)
mc3.feed.df$id <- as.numeric(mc3.feed.df$id)
mc.feed.all.df <- bind_rows(mc1.feed.fix.df,
                            mc2.feed.df,
                            mc3.feed.df,
                            .id = "rep")
...
mc3.drone.mass.df <- mc3.drone.mass.df %>%
  filter(id != 2.7)
mc.dronemass.all.df <- bind_rows(mc1.drone.mass.df,
                                 mc2.drone.mass.df,
                                 mc3.drone.mass.df,
                                 .id = "rep")
mc.dronemass.all.df <- mc.dronemass.all.df %>%
  mutate(trt_pulse = ifelse(id >= 3, 
                            "pulse",
                            "constant"),
         trt_total = ifelse(id < 2 | id > 4,
                            ...
                            "100"))
mc.dronemass.all.df <- mc.dronemass.all.df %>%
  mutate(avg.individ.mass = drone.mass / n.drones)
mc.dronemass.all.df$trt_total <- as.character(mc.dronemass.all.df$trt_total)
mc.droneit.all.df <- bind_rows(mc1.drone.it.df,
                               mc2.drone.it.df,
                               mc3.drone.it.df,
                               .id = "rep")
...
mc.feed.sum.df <- mc.feed.all.mod.df %>%
  ungroup() %>%
  group_by(rep, id, treatment) %>%
  summarise(psum = sum(p_mass_fd, na.rm = TRUE),
            nsum = sum(n_mass_fd, na.rm = TRUE)) %>%
  ungroup() %>%
  group_by(treatment) %>%
  summarise(m_psum = mean(psum),
            m_nsum = mean(nsum),
            sd_psum = sd(psum),
            sd_nsum = sd(nsum))

mc.feed.all.mod.df$id <- as.numeric(as.character(mc.feed.all.mod.df$id))
mc.feed.all.mod.df <- mc.feed.all.mod.df %>%
  mutate(trt_pulse = ifelse(id >= 3, 
                            "pulse",
                            "constant"),
         trt_total = ifelse(id < 2 | id > 4,
...
mc.feed.all.fix.df <- mc1.feed.fix.df %>%
  ...
  mutate(mc_mass_true = na.approx(mc_mass_true,
                                  maxgap = 2,
                                  rule = 3))
mc.feed.all.fix.df <- mc.feed.all.fix.df %>%
  filter(fd_day != 0)
```
## Export data
```{r}
write_csv(mc.all.npcons.df, "./")
write_csv(mc.end.all.df, "./microcol_final_endmass.csv")
write_csv(mc.feed.all.fix.df, "./microcol_final_feed.csv")
write_csv(mc.drone.all.df, "./microcol_final_drones.csv")
write_csv(mc.dronemass.all.df, "./microcol_final_dronemass.csv")
write_csv(mc.droneit.all.df, "./microcol_final_droneit.csv")
...
```{r}
mcall.pcons.lmm <- lme(total_p_cons ~ trt_pulse * trt_total + rep,
                       random = ~ 1 | id,
...
##### LS means Calculation
```{r}
mcall.pcons.marginal <- lsmeans(mcall.pcons.lmm, ~ trt_pulse * trt_total)
mcall.pcons.cld <- cld(mcall.pcons.marginal,
                       alpha = 0.05,
                       Letters = letters,
                       adjust = "tukey")
mcall.pcons.cld$.group <- gsub(" ", "", mcall.pcons.cld$.group)
mcall.pcons.cld <- mcall.pcons.cld %>%
  mutate(treatment = c("zone.1", "zone.4", "zone.3", "zone.2")) 
mcall.pcons.cld
```
...
##### Model specification/diagnostics
```{r}
mc.all.npcons.df$id <- as.numeric(as.character(mc.all.npcons.df$id))
mc.all.npcons.df <- mc.all.npcons.df %>%
  mutate(trt_pulse = ifelse(id >= 3, 
                            "pulse",
                            "constant"),
         trt_total = ifelse(id < 2 | id > 4,
                            "60",
                            "100"))
mcall.ncons.lmm <- lme(total_n_cons ~ trt_pulse * trt_total + rep,
                       random = ~ 1 | id,
                       data = mc.all.npcons.df,
                       na.action = na.exclude)
...
##### LS means Calculation
```{r}
mcall.ncons.marginal <- lsmeans(mcall.ncons.lmm, ~ trt_pulse * trt_total)
mcall.ncons.cld <- cld(mcall.ncons.marginal,
                       alpha = 0.05,
                       Letters = letters,
                       adjust = "tukey")
mcall.ncons.cld$.group <- gsub(" ", "", mcall.ncons.cld$.group)
mcall.ncons.cld <- mcall.ncons.cld %>%
  mutate(treatment = c("zone.1", "zone.4", "zone.3", "zone.2")) 
mcall.ncons.cld
```
...
##### Temporal correlation structure
```{r}
mcall.trtslope.model = lme(slope.estimate ~ treatment + pulse_cat_int, 
                           random = ~ 1 + treatment | id,
                           data = mc.all.trtslope.df,
                           na.action = na.exclude)
acf(mc.all.trtslope.df$slope.estimate)
ACF(mcall.trtslope.model)
```

##### Full model
```{r}
mcall.trtslope.model.1 = lme(slope.estimate ~ treatment + pulse_cat_int, 
                             random = ~ 1 + treatment | id,
                             data = mc.all.trtslope.df,
                             ...
                             na.action = na.exclude,
                             method = "REML")
summary(mcall.trtslope.model.1)
tidy(mcall.trtslope.model.1)
Anova(mcall.trtslope.model.1)
anova(mcall.trtslope.model.1)
```
...
mc.all.trtslope.df <- mc.all.trtslope.df %>%
  mutate(trt_pulse = ifelse(id >= 3, 
         ...
         trt_total = ifelse(id < 2 | id > 4,
                            "60",
                            "100"))
mcall.growth.lmm <- lme(slope.estimate ~ trt_pulse * trt_total + pulse_cat,
                        random = ~ 1 | id,
                        data = mc.all.trtslope.df)
mcall.growth.marginal <- lsmeans(mcall.growth.lmm,
                                 ~ trt_pulse*trt_total3)
...
#### LMM pollen per male
```{r}
mcall.pdrone.lmm <- lme(p_per_drone ~ trt_total * trt_pulse + rep,
                        random = ~ 1 | id,
...
```{r}
mcall.pdrone.marginal <- lsmeans(mcall.pdrone.lmm, ~ trt_total*trt_pulse)
...
mcall.pdrone.cld <- cld(mcall.pdrone.marginal,
                       ...
                       Letters = letters,
                       adjust = "tukey")
mcall.pdrone.cld$treatment <- c("zone.2", "zone.4", "zone.3", "zone.1")
mcall.pdrone.cld
```
...
mc.feed2drone.df$unique <- paste(mc.feed2drone.df$rep,
                                 mc.feed2drone.df$id,
                                 sep = ".")
mcall.ndrone.lmm <- lme(n_per_drone ~ trt_total * trt_pulse + rep,
                        random = ~ 1 | id,
                        data = mc.feed2drone.df,
                        na.action = na.exclude,
                        control=list(maxIter=10000, niterEM=10000))
...
```{r}
mcall.ndrone.marginal <- lsmeans(mcall.ndrone.lmm, ~ trt_total * trt_pulse)
...
mcall.ndrone.cld <- cld(mcall.ndrone.marginal,
                       ...
                       Letters = letters,
                       adjust = "tukey")
mcall.ndrone.cld$treatment <- c("zone.2", "zone.4", "zone.3", "zone.1")
mcall.ndrone.cld
```
...
contrast(mcall.mass.2way.marginal,
         alpha = 0.05,
         method = "pairwise",
         adjust = "tukey")
...
contrast(mcall.drone.2way.marginal,
         alpha = 0.05,
         method = "pairwise",
         adjust = "tukey")
...
mc.dronemass.all.df <- mc.dronemass.all.df %>%
  mutate(trt_pulse = ifelse(id >= 3, 
         ...
         trt_total = ifelse(id < 2 | id > 4,
                            "60",
                            "100"))
mcall.dronemass.lmm <- lm(avg_drone_mass ~ trt_total * trt_pulse + rep,
                           data = mc.dronemass.all.df,
                           na.action = na.exclude)
tidy(mcall.dronemass.lmm)
summary(mcall.dronemass.lmm)
Anova(mcall.dronemass.lmm)
anova(mcall.dronemass.lmm)
```

##### Plot LS means
```{r}
mcall.dronemass.marginal <- lsmeans(mcall.dronemass.lmm, ~ trt_pulse:trt_total)
contrast(mcall.dronemass.marginal,
         method = "pairwise",
         alpha = 0.05,
         adjust = "tukey")
mcall.dronemass.cld <- cld(mcall.dronemass.marginal,
                           alpha = 0.05,
                           Letters = letters,
                           adjust = "tukey")
mcall.dronemass.cld <- mcall.dronemass.cld %>%
  mutate(treatment = c("zone.1",
                       "zone.4",
                       "zone.2",
                       "zone.3"))
mcall.dronemass.cld
...
##### Plot LS means
```{r}
mcall.dronemass.cld$.group=gsub(" ", "", mcall.dronemass.cld$.group)
ggplot(data = mcall.dronemass.cld,
       mapping = aes(x = treatment,
                     y = lsmean,
                     label = .group)) + 
  geom_col(mapping = aes(fill = treatment),
